...
import com.example.onlincecoffeeshopapp.Helper.TinyDB
...
class ManagmentCart(val context: Context) {
    ...
    fun insertItems(item: ItemsModel) {
        ...
        if (existAlready) {
            listItem[index].numberInChart = item.numberInChart
        } else {
            listItem.add(item)
        }
    ...
    fun minusItem(listItems: ArrayList<ItemsModel>, position: Int, listener: ChangeNumberItemsListener) {
        if (listItems[position].numberInChart == 1) {
            listItems.removeAt(position)
        } else {
            listItems[position].numberInChart--
        }
